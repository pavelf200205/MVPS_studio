...
class Runner:
    def __init__(self, conf_text, mode='train', is_continue=False, datadir=None):
        ...
        self.progress_callback = None  # Optional: fn(current_iter, total_iter)
    ...
    def train(self):
        ...
        # Tensorboard graph generation fails with tinycudann because of device mismatch (CPU dummy tensor vs CUDA network).
        # self.writer.add_graph(self.sdf_network, verbose=False, input_to_model=torch.randn(1, 3).cuda())
        ...
        for iter_i in pbar:
            ...
            # Emit progress to the UI if a callback has been installed
            if self.progress_callback is not None:
                self.progress_callback(self.iter_step, self.end_iter)
        ...
        # Always extract the final mesh and save weights when the loop finishes or is stopped
        print("Training loop finished. Extracting final mesh...")
        self.validate_mesh(resolution=self.val_mesh_res)
        self.save_checkpoint()
