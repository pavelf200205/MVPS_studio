...
class ScaledImageLabel(QFrame):
    ...
    def paintEvent(self, event):
        ...
        if self._pixmap and not self._pixmap.isNull():
            ...
            scaled_pix = self._pixmap.scaled(rect.size(), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
            painter.drawPixmap(rect.x() + (rect.width() - scaled_pix.width()) // 2, rect.y() + (rect.height() - scaled_pix.height()) // 2, scaled_pix)
